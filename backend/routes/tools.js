...
router.post('/execute', optionalAuth, async (req, res) => {
  try {
    const userId = getUserIdFromRequest(req) || req.body.userId || 'default';
    ...
    const result = await executeTool(toolName, args, { userId, confirmed, pendingId });
...
router.post('/confirm', optionalAuth, async (req, res) => {
  try {
    ...
    const result = await confirmTool(pendingId, { approve: !!approve, userId });
